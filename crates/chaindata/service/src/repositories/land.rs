...
impl LandRepository {
    ...
    pub async fn save(&self, land: LandModel) -> Result<LandModelId, sqlx::Error> {
        query!(
            r#"
            INSERT INTO land (
    ...
    pub async fn get_by_id(&self, id: LandModelId) -> Result<Option<LandModel>, sqlx::Error> {
        query_as!(
            LandModel,
            r#"
            SELECT
...
mod tests {
    ...
    async fn test_save_and_get_land(pool: sqlx::PgPool) -> sqlx::Result<()> {
        let repo = LandRepository::new(pool);
